...
From this graph we see that in 2015 there were fewer departures than in 2016 or 2017. We also see that the percentage of National line delays are much lesser in 2015. 
...
garden_harvest_animate <-
  ...
  ggplot(aes(x = date, y = fct_reorder(variety, cum_weight_lb, .desc = TRUE)+
...
covid2<-
  ...
  filter(cases>20) %>% 
  ggplot()+
  geom_path(aes(x = cases, y = new_cases, group = state)) +
  geom_point(aes(x = cases, y = new_cases, color = state)) +
  geom_text(aes(x = cases, y = new_cases, label = state)) +
